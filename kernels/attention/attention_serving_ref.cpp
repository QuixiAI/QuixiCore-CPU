...
bool valid_paged(long long cache_blocks, long long batch,
...
Status paged_impl(const float* q, const KeyValue* key_cache,
  ...
  for (long long request = 0; request < batch; ++request) {
    ...
    for (long long position = 0; position < context; ++position) {
      const int physical = block_table[request * max_blocks + position / page_size];
...
Status quantized_attention(const float* q, const void* packed_k,
  ...
  threading::parallel_ranges(batch * heads * sequence, 1,
                             [&](long long begin, long long end, int) {
    for (long long item = begin; item < end; ++item) {
      ...
      for (long long key_position = 0; key_position < key_count; ++key_position) {
        const std::uint8_t* key =
            keys + static_cast<std::size_t>(prefix * sequence + key_position) *
                       vector_bytes;
        double dot = 0.0;
        for (long long dim = 0; dim < head_dim; ++dim) {
          dot += query[dim] * quant::gguf_dequant_element(
                                  format,
      ...
      for (long long key_position = 0; key_position < key_count; ++key_position) {
        const std::uint8_t* value =
            values + static_cast<std::size_t>(prefix * sequence + key_position) *
                         vector_bytes;
        const double probability = scores[key_position] / denominator;
        for (long long dim = 0; dim < head_dim; ++dim) {
          output[dim] += static_cast<float>(
              probability * quant::gguf_dequant_element(
                                format,
...
Status rope_q_norm(const float* q, const float* cosine, const float* sine,
  ...
  for (long long token = 0; token < tokens; ++token) {
    for (long long head = 0; head < heads; ++head) {
      ...
      const float inverse = do_norm
                                ? 1.0f / std::sqrt(square_sum / head_dim + eps)
                                : 1.0f;
      for (long long dim = 0; dim < half; ++dim) {
        const float w0 = do_norm
                             ? norm_weight[dim] + (gemma_weight ? 1.0f : 0.0f)
                             : 1.0f;
        const float w1 = do_norm
                             ? norm_weight[dim + half] +
                                   (gemma_weight ? 1.0f : 0.0f)
                             : 1.0f;
...
Status rope_kv_insert(const float* k, const float* v, const float* cosine,
                      ...
                      long long max_position, bool do_norm,
                      bool gemma_weight, float eps) {
  ...
  std::vector<float> rotated(static_cast<std::size_t>(tokens * kv_heads * head_dim));
  ...
  for (long long token = 0; token < tokens; ++token) {
    ...
    std::copy_n(v + token * kv_heads * head_dim, kv_heads * head_dim,
...
Status paged_attention_advanced(
    ...
    long long page_size, long long max_blocks, float scale,
    long long window, float softcap) {
...
Status paged_attention_fp8(
    ...
    Float8Format format, float scale, long long window, float softcap) {
  return paged_impl(q, key_cache, value_cache, block_table, context_lens,
...
Status paged_attention_xcache(
    ...
    long long max_blocks, long long vector_width, float scale) {
  if (!valid_paged(cache_blocks, batch, query_heads, kv_heads, head_dim,
  ...
  return paged_attention(q, standard_key.data(), standard_value.data(),
                         ...
                         query_heads, kv_heads, head_dim, page_size,
                         max_blocks, scale);
}
    ...
    for (long long item = begin; item < end; ++item) {
      ...
      for (long long position = 0; position < total; ++position) {
        ...
        if (position < prefix_length) {
          ...
          const int physical = block_table[
              request * max_blocks + local / page_size];
          key = key_cache + ((static_cast<long long>(physical) * page_size +
                              local % page_size) * kv_heads + kvhead) * head_dim;
        }
        double dot = 0.0;
        for (long long dim = 0; dim < head_dim; ++dim) dot += query[dim] * key[dim];
      ...
      for (long long position = 0; position < total; ++position) {
        ...
        if (position < prefix_length) {
          ...
          const int physical = block_table[
              request * max_blocks + local / page_size];
          value = value_cache + ((static_cast<long long>(physical) * page_size +
                                  local % page_size) * kv_heads + kvhead) * head_dim;
        }
...
Status cascade_attention_fp8(
    ...
    long long max_blocks, Float8Format format, float scale) {
  if (!valid_paged(cache_blocks, batch, query_heads, kv_heads, head_dim,
                   page_size, max_blocks, scale, 0, 0.0f) ||
      prefix_length <= 0 ||
  ...
  if (!detail::all_nonnull(q, prefix_k, prefix_v, key_cache, value_cache,
                           block_table, context_lens, key_scale,
                           value_scale, out)) {
    return Status::kInvalidArgument;
  }
  for (long long head = 0; head < kv_heads; ++head) {
    ...
    for (long long head = 0; head < kv_heads; ++head) {
      for (long long dim = 0; dim < head_dim; ++dim) {
        const long long index =
            (position * kv_heads + head) * head_dim + dim;
        decoded_k[index] = float8_decode(prefix_k[index], format) *
                           key_scale[head];
        decoded_v[index] = float8_decode(prefix_v[index], format) *
                           value_scale[head];
      }
    }
  }
  return cascade_attention(
      q, decoded_k.data(), decoded_v.data(), key_cache, value_cache,
      block_table, context_lens, out, cache_blocks, batch, query_heads,
      kv_heads, head_dim, prefix_length, page_size, max_blocks, scale);
}
